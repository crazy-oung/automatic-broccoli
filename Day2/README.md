...
- **feed DOM트리가 복잡해지면 발생하는 현상, jquery로 직접 dom을 select할 경우(Ex: $("#test") 복잡할 수록 트리이기 때문에, 검색 비용이 증가함, 정확히는 줄일 수가 없음.(트리 탐색 알고리즘 한계) 이 사실을 알고 있어야하고, 그렇기 때문에 최대한 DOM select하는 케이스를 줄이는게 조금이나마 렌더링 비용을 줄일 수 있게됨**
...
- 주로 TCP를 사용하고 HTML문서를 주고받는데 사용
  - **feed tcp / udp 의 차이도 알 필요가 있음**
- 요청(Request)과 응답(Response)으로 구성
  - 클라이언트가 서버에 정보를 요청하면 응답 코드와 내용을 전송하고 클라이언트와 연결을 종료
- 전송할 때 일반 텍스트 타입으로 전송됨
  - **feed 흠.. phy계층에서는 텍스트는 절대 아니고, 신호로 전달됨(ex 빛, 전기 등) 이건 OSI7계층에 대한 이해가 필요함**

### 브라우저가 HTTP방식으로 서버로부터 데이터를 받는 과정
> [네트워크](#4-osi-7-layer-계층별-역할)에 대한 이해가 필요함 - 굳
...
- 서로 다른 쌍의 키로 암복호화를 진행함
  ...
  - 공개키로는 암호화된 정보를 복호화 할 수 없으므로 보안 문제가 해결됨
    - **feed 이것도 진짜 면접 단골 질문이라, 어떤 방식으로 암호화가되는지 설명할 수 있어야함.**

### 신뢰할 수 있는 사이트?
- 공개키를 가지고 
  - 신뢰할 수 있는 기관에서 이를 검증해줘야함
    - 검증받으면 CA(Certificate Authority)가 생성됨
    - CA는 브라우저에 내장되어 있음 // **feed 엥 이거 맞음? CA서버가 따로 있을텐데, 그래서 https 인증서를 돈주고 사는것일거고.. 내가 틀렸을 수도 있으니 확인바람 ㅋ 나도 오래되서 기억이 가물가물** 
  - 탐색과정(핸드쉐이크)를 통해 검증
...
- 암호화될 필요가 없는 데이터만 있을 경우 HTTP 사용
  - **feed 이건 불가능함, https 사이트에서 http 통신을 하면 보안 위협된다고 에러뜸**
- 모든 통신에 HTTPS 를 사용할 경우 암호화가 불필요한 정보도 암호화 하게되므로 부하가 높고 효율적이지 못함
  - 그러므로 두 방식을 혼용해서 사용함 
    - **feed 여기서 방식은 http만 쓰는 페이지도 있고, https만 쓰는 페이지가 있다는 말이겠지?**

---
## 3. web server의 역할
### web server?
- HTML, CSS, IMG 등과 같이 정적인 요소의 요청을 받아 처리하는 곳
  - **feed 흠..처리? SSR경우는 처리를 한다만, 일반적으로는 저장하는 곳이 맞음**
...
- WAS와 웹서버를 분리해서 부하를 덜고 장애를 예방함
  - **feed 로드밸런싱과 L4를 통한 장애 미연감지 정도라 볼 수 있음, 여기까지 알 필요는 없지만, L4 switch에 대한 공부도 해두면 언젠가는 쓸 때가 있을 것임 ㅋㅋ**
...
### OSI 7계층의 PDU
**feed osi 7계층에 대한 피드백은 따로 하지 않겠음. 정리가 잘된 자료들이 많을 것이고 기사시험에 나오는거라 공부하기 수월할 것임**
...
- 클라이언트가 요청을 하면 JSON타입의 데이터를 받을 수 있게 만드는 것
  - **feed 정의가 이게 아닐텐데 `자원을 정의하고 자원에 대한 주소를 지정하는 방법`에 대한 것들이 정의임, ex 음원파일을 webserver에 요청하면 어떻게 json타입으로 줄 수 있겠니?ㅋㅋ**
...
1. 표준화된 형식으로 정보를 전송할 수 있도록 구성 요소 간 **통합된(일관적인) 인터페이스** 필요
  - **feed 이게 제일 핵심이지만 제일 어려움**
2. 요청하는 서버에 클러이언트의 콘텐츠가 저장되지 않는 **무상태성(stateless)**
      - 세션 정보가 클라이언트에 저장됨 
      - **feed 흠..2,3,4번은 아닐텐데.. 이건 나중에 토론을 해봅시다. 원래 restful api 원칙은 정답이 없어야함. why? 실제로 완벽하게 rest하게 실서비스에 적용시키기에는 무리가 있고, 이 원칙을 지키는 것보다 서버 리소스를 줄이는게 더 합리적이기 때문.** 
